...
> 最后更新：2026-08-14，基于 `main` 分支 `8b0437b`（工作树干净，已推送至 origin/main）。
...
- **零依赖、零构建**：只有 HTML / CSS / 原生 JS，没有 package.json、没有框架；已加 GitHub Actions CI（仅跑 Node 测试，不含自动部署 Pages）。
- 依据《学生素质综合测评办法》实现综测计算，**仅供学生自评参考**，结果以班级测评小组、系及学院认定为准。
- 双页面互链：`index.html`（综测）↔ `transfer.html`（转专业），导航栏互相跳转。
- 站点 `<title>` 为「WMU综测转专业计算器」，部署于自定义域名 `wmu.calc.acretiondisk.top`。
...
| `test/storage.test.js`       | 存储层单测（Node）                                |
| `scripts/verify-transfer.js` | Playwright 渲染冒烟验证（**可选**，依赖本机 playwright） |
| `CNAME`                      | 自定义域名 `wmu.calc.acretiondisk.top`                |
| `resource/`                  | 图片资源（校徽长图等）                              |
...
| 新增综测学院方案 | ① `data.js` 定义常量 + `SCHEMES` 注册；② `index.html` 分段单选加一个 radio 项；③ `storage.js` scheme 白名单加 key | 测试                         |
...
> 2026-08-13 接手后已修复坑 1~4、坑 6（补 CI），并额外修了第 11 节列出的几处。坑 5、7 仍待处理；坑 8（工作树未提交）已解决——本次改动已全部提交并推送至 origin/main。
...
node test/storage.test.js   # 存储层，必须全绿
...
## 14. 其他落地改动（2026-08-14：方案切换 / 档案卡 / 标题）

- 方案切换 UI 由下拉改为页头**分段单选**（`#scheme-select` radiogroup），选择自动记忆，刷新沿用上次方案。
- 新增**个人档案卡**（`ZCArchive`，key `wmu-zongce-archives-v1`）：保存/命名/载入/导出/删除，支持导入他人分享的档案卡 JSON；载入后改动可一键「同步到当前档案」写回（`#btn-archive-sync`）。
- 校徽改用长图（`resource/红色透明底长图.png`）；C1/C2/C3 三卡等宽布局。
- 站点 `<title>` 统一为「WMU综测转专业计算器」。
- 新增 `CNAME`，站点迁移至自定义域名 `wmu.calc.acretiondisk.top`（细节见第 13 节）。
